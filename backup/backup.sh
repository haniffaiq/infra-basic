#!/bin/sh
# Dumps every app Postgres database plus grafana, snapshots Redis, mirrors all
# MinIO buckets
...
# MinIO bucket, so it stays out of APPS or `mc mirror` would fail on a bucket
# that does not exist. It is also absent until scripts/setup-monitoring-db.sh
# has run, so a missing grafana database is skipped rather than failing the run.
PG_DATABASES="$APPS"
if psql -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U postgres -d postgres -Atc \
     "select 1 from pg_database where datname = 'grafana';" 2>/dev/null | grep -q 1; then
  PG_DATABASES="$APPS grafana"
else
  log "postgres: skipping grafana (database not present — run scripts/setup-monitoring-db.sh)"
fi
